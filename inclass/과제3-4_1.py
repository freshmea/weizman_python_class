...
# 기후통계분석 란의 기온분석을 클릭하고 기간을 정하고 지역을 정한 다음 csv 포맺으로 다운로드 합니다.
# csv 파일을 열어서 5번째 칸 까지 삭제를 합니다.
# 파일 이름을 ta.csv 로 저장하고 코딩을 완료한 후에 실행 합니다.
...
for row in data:
    print(row)
    if row[-1] != '':
        if row[0].split('-')[1] == '11' and row[0].split('-')[2] == '26':
